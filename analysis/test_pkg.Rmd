...
# Point Pattern
```{r}
harran <- read.table("../mytestpkg/data/ReReLA/data/Sites_HarranPlain.csv", sep=",", header=TRUE)
head(harran)
...
#set.seed(123)
harran_poispp1 <- rpoispp(lambda = dens, win = harran_ppp2$window)   # poisson - complete spatial randomness 

## or
dens2 <- intensity(harran)
#set.seed(123)
harran_poispp2 <- rpoispp(lambda = dens, win = harran_ppp2$window)
